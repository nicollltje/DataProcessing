...
def extract_tvseries(dom):
    ...
    for item in super_parent:
        ...
        # The title.
        title = plaintext(item.by_tag("a")[0].content.encode('ascii', 'ignore'))
        print title
        
        # The actors
        actors = plaintext(item.by_tag("p.")[2].content.encode('ascii', 'ignore'))
        actors = actors[6:]
        print actors
...
def save_csv(f, tvseries):
    ...
    for x in range (0, len(tvseries)):
        writer.writerow(tvseries[x])
