...
d3.json("weatherdata1994.json", function(data){
	...
	// make a line from the datapoints for minimum temperature
	...
	// make a line from the datapoints for maximum temperature
	...
	// make a line from the datapoints for average temperature
	...
	d3.selectAll(".m")
		.on("click", function() {
			...
			// set the domains for the graph
			x.domain(d3.extent(data, function(d) { if(d.city == str){
				return d.date; }}));
			y.domain([
				d3.min(data, function(d){return d.minTemperature}),
				d3.max(data, function(d) { if(d.city == str){
					return d.maxTemperature; }})
			]);

			// make a line from the datapoints for minimum temperature
			var lineMin = d3.svg.line()
				.x(function(d) {if(d.city == str){
					return x(d.date); } else{return x.domain} })
				.y(function(d) {if(d.city == str){
					return y(d.minTemperature); } else{return y.domain} });
	...
	var focus = svg.append("g")
  	...
  	function mousemove() {
    	...
    	focus.select("text").text((d.date, d.avgTemperature));
  }
